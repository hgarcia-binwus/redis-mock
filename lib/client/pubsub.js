  ...
  for (let i = 0; i < arguments.length; i++) {
    ...
    if ('string' === typeof arguments[i]) {
      ...
      process.nextTick(function () {
        ...
        if (listener) {
          self.subscriptionsListeners[channelName] = listener;
          self.on('message', (channel, msg) => {
            if (self.subscriptionsListeners[channel]) {
              self.subscriptionsListeners[channel](msg, channel);
            }
          });
        }
...
exports.publish = function (mockInstance, channel, msg) {
  ...
  this.pub_sub_mode = true;
  process.nextTick(function () {
    if ((typeof msg === "object" && !Buffer.isBuffer(msg)) && (msg !== null)) {
